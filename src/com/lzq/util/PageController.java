...
public class PageController {
	...
	{// 初始化代码块
		if(PageController.bigList==null) {
			PageController.bigList=getSelectAll("select * from player");// 调用查询数据库的方法，返回所有的行
		}
		//获取总页数
	...
	public static Vector<Vector> getSelectAll(String sql){
		...
		try(Connection conn=conTool.getconnection();//获取数据库接
			...
			while(rs.next()) {//遍历数据集
				...
				row.add(rs.getString(3));//获取第二个字段姓名
				
				row.add(rs.getString(4));//获取第一个字段学号
				row.add(rs.getString(5));//获取第二个字段姓名
				row.add(rs.getString(6));//获取第二个字段姓名
				row.add(rs.getString(7));//获取第二个字段姓名
